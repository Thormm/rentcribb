...
const Overview = () => {
  ...
  useEffect(() => {
    ...
    fetch(PROFILE_FETCH_URL, {
      ...
      body: JSON.stringify({ action: "overviewfetchStudent", whats }),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const saveProfile = async () => {
    ...
    form.append("action", "overviewupdate_profile_student");
  ...
  const saveNextOfKin = async () => {
    ...
    const payload = {
      action: "overviewupdate_kin_student",
      user,
      signup_key,
      kin_first: kinFirst,
      kin_last: kinLast,
  ...
  const handleSendFeedback = async () => {
    ...
    try {
      const res = await fetch(SAVE_URL, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "overview_feedback_send_student",
          user,
          feedback,
        }),
      });
